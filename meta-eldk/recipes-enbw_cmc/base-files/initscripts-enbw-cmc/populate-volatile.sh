...
create_file() {
	EXEC="
	touch \"$1\";
	chown ${TUSER}.${TGROUP} $1 || echo \"Failed to set owner -${TUSER}- for -$1-.\" >$vtmaster 2>&1;
	chmod ${TMODE} $1 || echo \"Failed to set mode -${TMODE}- for -$1-.\" >$vtmaster 2>&1 "
...
link_file() {
	EXEC="test -e \"$2\" -o -L $2 || ln -s \"$1\" \"$2\" >$vtmaster 2>&1"
